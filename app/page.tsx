...
export default function Home() {
  ...
  const handleDeleteTodo = async (todoId: number) => {
    const parentsToUpdate = dagManager.getParentIds(todoId);
    ...
    try {
      ...
      await fetch('/api/todos/delete-todo', {
        ...
        body: JSON.stringify({ 
          deletedId: todoId,
          parentsToUpdate: parentsToUpdate
        }),
      })
      .then(async response => {
        if (!response.ok) {
  ...
  return (
    <div className="w-full h-screen bg-gray-50 relative bg-gradient-to-b from-indigo-300 to-indigo-700">
      ...
      <div className="absolute top-6 right-6 z-10 bg-white rounded-lg shadow-lg border max-w-md">
        <TodoForm
          onRefreshList={() => setRootTodos(dagManager.getTodosByIds(dagManager.getIndegreeZeroNodes))}
